...
participant_info <- fulldata %>%
  ...
  select(-c(selfrate_max, selfrate_min))
...
fulldata %>%
  filter(eye_exclude == FALSE) %>%
  group_by(maingroup, direction, participant, age) %>%
  summarise(facechest = mean(facechest, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(maingroup = case_when(
    maingroup == "DeafEarly" ~ "Deaf Early",
  ...
  write_csv("../Study2Children/fcr_individual_values_adults.csv")

facechest_info %>%
  write_csv("../Study2Children/fcr_chart_adults.csv")
