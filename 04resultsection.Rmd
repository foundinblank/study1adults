...
datafc <- dataoriginal
model <- lmer(acc ~ percent * direction + (direction|id), data = filter(datafc,aoi=="mouth" & maingroup!="NoviceASL"))
model2 <- lm(acc ~ percent * direction, datafc) # datafc filtered to mouth AOI only
summary(model)
plot(model)
library(ggfortify)
autoplot(model2)
